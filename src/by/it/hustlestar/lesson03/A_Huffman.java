package by.it.hustlestar.lesson03;
...
public class A_Huffman {
    ...
    String encode(File file) throws FileNotFoundException {
        ...
        for (int i = 0; i < chars.length; i++) {
            ...
            try {
                counter = count.get(chars[i]);
            } catch (Exception e) {
                counter = 0;
            }
            count.put(chars[i], ++counter);
        }
        //2. перенесем все символы в приоритетную очередь в виде листьев
        PriorityQueue<Node> priorityQueue = new PriorityQueue<>();
        for (Map.Entry<Character, Integer> characterIntegerEntry : count.entrySet()) {
            priorityQueue.add(new LeafNode(characterIntegerEntry.getValue(), characterIntegerEntry.getKey()));
        }
